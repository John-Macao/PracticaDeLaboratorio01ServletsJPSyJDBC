...
Una vez presionamos en “Registrar Compra” esta se dirige al controlador y ejecuta RegistrarCompraController.java podemos observarlo en la parte superior, el cual recibe los parámetros del usuario como referencia del usuario actual, posteriormente se manda a buscar la última cabecera creada para así mismo tenerla como referencia para los detalles. Seguido mandamos a traer el listado de las categorías con el método implementado en la clase JDBCCategoriaDAO. Dentro del try-catch se llama una lista de los productos actuales que se ejecuta en la clase JDBCProductoDAO, tomando los productos que no estén con un estado de eliminado, posteriormente se ejecuta un bucle for para la generar una nueva lista con productos ligados con su categoría, finalmente se envían los parámetros requeridos mediante el request.setAttribute. 
...
Para agregar un detalle mandamos a setear una el objeto detalle con la cantidad recibida anteriormente, luego mandamos el objeto detalle, la cabecera, y el id del usuario al método crear() dentro de la clase JDBCDetalleDAO, posterior se vuelve a realizar el llamado de los productos. 
...
Una vez obtenemos los parámetros que necesitamos procedemos a invocar los diferentes métodos del JDBC que nos ayudan en la obtención de información según sea requerida, en este caso invocamos un método para traer una lista de todos los detalles según la cabecera enviada, así mismo mandamos a traer una lista de productos para posteriormente mandar a buscar un detalle con el id del producto recibido. 
...
Una vez seleccionado el producto y especificado la cantidad a ser modificado se envían esos parámetros al controlador de “ModificarDetalleController2” donde se invocan a los diferentes métodos para traer la información necesaria y mandar a modificar el detalle con la información actualizada, en este caso para realizar dicho proceso se invoca al método de “update” dentro de la clase JDBCDetalleDAO que recibe como parámetro un objeto del tipo detalle.  
...
Dentro del controlador “EliminarCompraController2” nos permite realizar la actualización del estado de una compra realizada cambiando el estado por “D” que hace referencia a una compra eliminada, para ello primero creamos un objeto Cabecera y lo seteamos con un estado “D” para posteriormente enviar al metodo “update” que está dentro de JDBCCabeceraDAO para la actualización de información de la cabecera.  
...
### BuscarProductosController.java 
...
Para realizar la conexión a la base de datos se trabaja con JDBC, el cual en conjunto con los métodos que se hayan especificado tanto en las Interfaces específicas, como en la Interfaz Generic, permitirá que se hagan consultas a la Base de Datos por medio de sentencias SQL. Para realizar la conexión se crea la clase ContextJDBC, donde se especifican el Driver, URL, usuario y contraseña de la Base con la que se piensa trabajar, además de los métodos que permiten esta conexión. 
...
read2(String cat_nombre): Categoria -> Método que devuelve un objeto categoría según el nombre de la categoría relacionada. Usado en el controlador TEST. 
...
crear(Producto producto, int empresa_id, int categoría_id): void -> Método para registrar un producto con su respectivas llaves foráneas, usado en el controlado RegistrarProductosController. 
...
find(int empresa_id): List<Producto> -> Método que devuelve una lista de productos según la empresa con la que está relacionado el usuario que Inicio Sesion. Usado en controladores como ModificarProductosController, BuscarProductosController, ListarProductosController y ListarEmpresas. 
...
buscarSoloPorNombre(String pro_nombre): Producto -> Método que devuelve un objeto producto según su nombre con la que el usuario está relacionado. Usado en el controlador de ModificarDetalleController2 y TEST. 

### JDBCProductoDAO 

![img](/imagenes/i58.png)

TEST(int producto_id): Producto -> Método que devuelve un objeto producto según el id del producto con el cual está relacionado. Usado en controlador ListarCompraController2, ListarCompraController3, ModificarDetalleController y ModificarDetalleController2. 

### JDBCProductoDAO 

![img](/imagenes/i59.png)

buscarPorCategoria2(int categoria_id): List<Producto> -> Método que devuelve una lista de tipo producto según el id de la cabecera relacionado y el estado sea diferente de “e”. Se utiliza en el controlador de TEST 

### JDBCProductoDAO 

![img](/imagenes/i60.png)

obtenerCategoriaId(int product_id): int -> Método que devuelve el id de una categoria en valor entero según el id de un producto que esté relacionado. Se utiliza en los controladores ModificarDetalleController, ModificarDetalleController2, ListarCompraController2 y ListarCompraController3. 
...
crear(Cabecera cabecera, int usuario_id): void -> Método para registrar una cabecera con sus respectivas llaves foráneas, usado en el controlado RegistrarCompraController. 
...
ultimoCreado(): int -> Método para obtener el id de la última cabecera creada. Usado en el controlador RegistrarCompraController. 
...
listarSinDelete(int usuario_id): List<Cabecera> -> Método para obtener una lista de cabeceras según el usuario relacionado y con estado diferente de “D”. Usado en el controlador de ListarCompraController. Devuelven cabeceras que tiene como estado “Aceptado”, “Rechazado” y “Espera”. 
...
crear(Detalle detalle, int cabecera_id, int producto_id): void -> Método para registrar un detalle con sus respectivas llaves foráneas, usado en el controlado TEST. 
...
obtenerProductoId(Detalle detalle): int -> Método que devuelve el id de un producto según este relacionado. Usado en los controladores ListarCompraController2, ListarCompraController3, ModificarDetalleController y ModificarDetalleController2. 

### JDBCDetalleDAO 

![img](/imagenes/i69.png)

test(int cabecera_id): Detalle -> Método que devuelve un objeto detalle según el id de la cabecera enviado. Usado en controlador TEST. 

### JDBCDetalleDAO 

![img](/imagenes/i70.png)

test2(int producto_id, int cabecera_id): Detalle -> Método que devuelve un objeto detalle según el id de un producto y id de una cabecera. Usado en el controlador ModificarDetalleController2. 
